...
def exploracion_paciente():
    cursor = mysql.connection.cursor()
    cursor.execute('SELECT id_paciente, paciente FROM tb_pacientes')
    ...
    if request.method == 'POST':
        ...
        latidos_por_minuto = request.form['latidos_por_minuto']
        glucosa = request.form['glucosa']
        edad = request.form['edad']
        cursor.execute('''
            INSERT INTO tb_exploraciones (
                id_paciente, fecha, peso, altura, temperatura, latidos_por_minuto, glucosa, edad
            ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
        ''', (id_paciente, fecha, peso, altura, temperatura, latidos_por_minuto, glucosa, edad))
        mysql.connection.commit()
        cursor.close()
        flash('Exploración guardada exitosamente.')
